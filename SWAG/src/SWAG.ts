...
class SWAG implements IPreAkiLoadMod, IPostDBLoadMod {
	...
	static ReadAllPatterns(): void{
		...
		let files = fs.readdirSync(`${dirpath}/config/patterns/`);
		for (let file of files){
			let temppattern = require(`${dirpath}/config/patterns/${file}`);
			let tempname = file.split(".")[0];
			//parse the json and push it to the customPatterns array
			customPatterns[tempname] = jsonUtil.deserialize(temppattern);
	...
	static ConfigureMaps(): void {
		...
		for (let globalmap in locations) {
			//read mapWrapper in locations and set its values to be used locally
			for (let pattern in customPatterns) {
				//read mapWrapper in pattern and set its values to be used locally
				let mapWrapper: ClassDef.MapWrapper = customPatterns[pattern][0];
				let mapName: string = mapWrapper.MapName.toLowerCase();
				let mapGroups: ClassDef.GroupPattern[] = mapWrapper.MapGroups;
				let mapBosses: ClassDef.BossWave[] = mapWrapper.MapBosses;
	...
	static SetUpGroups(mapGroups: ClassDef.GroupPattern[], mapBosses: ClassDef.BossWave[], globalmap: string): void {

		//set up local variables to contain outside of loop
		let RandomGroups: ClassDef.GroupPattern[] = [];
		let RandomBossGroups: ClassDef.BossWave[] = [];
		let StaticGroups: ClassDef.GroupPattern[] = [];
		let StaticBossGroups: ClassDef.BossWave[] = [];
		let AlreadySpawnedGroups: ClassDef.GroupPattern[] = [];
		let AlreadySpawnedBossGroups: ClassDef.BossWave[] = [];

		//read mapGroups and see if value Random, OnlySpawnOnce, or BotZone is set and set local values
		for (let group of mapGroups) {
			let groupRandom: boolean = group.RandomTimeSpawn;
		...
		for (let boss of mapBosses) {
			let groupRandom: boolean = boss.RandomTimeSpawn;
			let groupOnlySpawnOnce: boolean = boss.OnlySpawnOnce;
	...
	static SetUpRandomBots(RandomGroups: ClassDef.GroupPattern[], globalmap: string, AlreadySpawnedGroups: ClassDef.GroupPattern[]): void {

		//read a random group from RandomGroups
		let randomGroup = randomUtil.getArrayValue(RandomGroups);
		let RandomGroupSpawnOnce: boolean = randomGroup.OnlySpawnOnce;
		let RandomGroupBotZone: string = randomGroup.BotZone;

		//if RandomGroupSpawnOnce is true, push group to AlreadySpawnedGroups and spawn individual bots
		if (RandomGroupSpawnOnce) {
			SWAG.SpawnRandomBots(randomGroup, globalmap, RandomGroupBotZone, RandomGroupSpawnOnce, AlreadySpawnedGroups);
	...
	static SetUpRandomBosses(RandomBossGroups: ClassDef.BossWave[], globalmap: string, AlreadySpawnedBossGroups: ClassDef.BossWave[]): void {

		//read a random group from RandomBossGroups
		let randomBossGroup = randomUtil.getArrayValue(RandomBossGroups);
		let RandomBossGroupSpawnOnce: boolean = randomBossGroup.OnlySpawnOnce;
		let RandomBossGroupBotZone: string = randomBossGroup.BossZone;

		//if RandomGroupSpawnOnce is true, push group to AlreadySpawnedGroups and spawn individual bots
		if (RandomBossGroupSpawnOnce) {
			SWAG.SpawnRandomBosses(randomBossGroup, globalmap, RandomBossGroupBotZone, RandomBossGroupSpawnOnce, AlreadySpawnedBossGroups);
	...
	static SetUpStaticBots(StaticGroups: ClassDef.GroupPattern[], globalmap: string, AlreadySpawnedGroups: ClassDef.GroupPattern[]): void {
		//read StaticGroups and set local values
		for (let group of StaticGroups) {
			let StaticGroupSpawnOnce: boolean = group.OnlySpawnOnce;
			let StaticGroupBotZone: string = group.BotZone;

			//if StaticGroupSpawnOnce is true, push group to AlreadySpawnedGroups and spawn individual bots
			if (StaticGroupSpawnOnce) {
				SWAG.SpawnStaticBots(group, globalmap, StaticGroupBotZone, StaticGroupSpawnOnce, AlreadySpawnedGroups);
	...
	static SetUpStaticBosses(StaticBossGroups: ClassDef.BossWave[], globalmap: string, AlreadySpawnedBossGroups: ClassDef.BossWave[]): void {
		...
		for (let boss of StaticBossGroups) {
			...
			if (StaticBossGroupSpawnOnce) {
				SWAG.SpawnStaticBosses(boss, globalmap, StaticBossGroupBotZone, StaticBossGroupSpawnOnce, AlreadySpawnedBossGroups);
				AlreadySpawnedBossGroups.push(boss);
			}
			//if StaticGroupSpawnOnce is false, spawn individual bots
			else {
	...
	static SpawnRandomBots(group: ClassDef.GroupPattern, globalmap: string, RandomGroupBotZone: string, RandomGroupSpawnOnce: boolean, 
		AlreadySpawnedGroups: ClassDef.GroupPattern[]): void {
		...
		for (let bot of group.Bots) {
			let wave: ClassDef.Wave = SWAG.ConfigureBotWave(group, bot, RandomGroupBotZone, globalmap);

			locations.base.Wave.push(wave);
		}
	}

	static SpawnRandomBosses(boss: ClassDef.BossWave, globalmap: string, RandomBossGroupBotZone: string, RandomBossGroupSpawnOnce: boolean,
		AlreadySpawnedBossGroups: ClassDef.BossWave[]): void {
		...
		if (boss.Supports != null) {
			for (let escort of boss.Supports) {
				escort.BossEscortDifficult = SWAG.diffProper[config.aiDifficulty.toLower()];
			}
		}

		//read group and create wave from individual boss but same timing and location if RandomBossGroupBotZone is not null
		
		let wave: ClassDef.BossWave = SWAG.ConfigureBossWave(boss, RandomBossGroupBotZone, globalmap);

		locations.base.BossLocationSpawn.push(wave);
		
	}


	static SpawnStaticBots(group: ClassDef.GroupPattern, globalmap: string, StaticGroupBotZone: string, StaticGroupSpawnOnce: boolean,
		AlreadySpawnedGroups: ClassDef.GroupPattern[]): void {
		...
		for (let bot of group.Bots) {
			let wave: ClassDef.Wave = SWAG.ConfigureBotWave(group, bot, StaticGroupBotZone, globalmap);

			locations.base.Wave.push(wave);
		}
	}

	static SpawnStaticBosses(boss: ClassDef.BossWave, globalmap: string, StaticBossGroupBotZone: string, StaticBossGroupSpawnOnce: boolean,
		AlreadySpawnedBossGroups: ClassDef.BossWave[]): void {
		...
		let wave: ClassDef.BossWave = SWAG.ConfigureBossWave(boss, StaticBossGroupBotZone, globalmap);

		locations.base.BossLocationSpawn.push(wave);

	}

	static ConfigureBotWave(group: ClassDef.GroupPattern, bot: ClassDef.Bot, RandomGroupBotZone: string, globalmap: string): ClassDef.Wave 
	{
		...
		wave.slots_min = 1;
		wave.slots_max = Math.floor(bot.MaxBotCount * SWAG.aiAmountProper[config.AIAmount]);
		wave.BotPreset = SWAG.diffProper[config.aiDifficulty.toLower()];
		wave.SpawnPoints = (RandomGroupBotZone != null) ? RandomGroupBotZone : this.getRandomStringArrayValue(SWAG.mappedSpawns[globalmap]);
	...
	static ConfigureBossWave(boss: ClassDef.BossWave, RandomBossGroupBotZone: string, globalmap: string): ClassDef.BossWave
	{
		let wave: ClassDef.BossWave = new ClassDef.BossWave();
		wave.BossName = SWAG.roleCase[boss.BossName];
		wave.BossChance = boss.BossChance;
		wave.BossZone = (RandomBossGroupBotZone != null) ? RandomBossGroupBotZone : this.getRandomStringArrayValue(SWAG.mappedSpawns[globalmap]);
		wave.BossPlayer = false;
		wave.BossDifficult = SWAG.diffProper[config.aiDifficulty.toLower()];
		wave.BossEscortType = boss.BossEscortType;
		wave.BossEscortDifficult = SWAG.diffProper[config.aiDifficulty.toLower()];
		wave.BossEscortAmount = boss.BossEscortAmount;
		wave.Time = boss.Time;
		wave.Supports = boss.Supports;
		wave.RandomTimeSpawn = boss.RandomTimeSpawn;
		wave.OnlySpawnOnce = boss.OnlySpawnOnce;
