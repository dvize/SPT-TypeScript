...
export class POOPDifficulty
{
	static readAITemplate(): AITemplate[] {
		...
		fs.readdirSync(directoryPath).forEach(function (file) {
			...
			if (template != null && template.RoleTypes.length > 0 && template.AIDifficultyModifier != null && template.AIDifficultyModifier != undefined && template.difficulty != null && template.difficulty != undefined) {
    ...
    static SetRogueNeutral() {
		...
		if (gv.config.AIChanges.RoguesNeutralToUsecs) {
			...
			for (let rogue in RogueTypes) {
				gv.botTypes[RogueTypes[rogue]].difficulty.easy.Mind.DEFAULT_ENEMY_USEC = false;
				gv.botTypes[RogueTypes[rogue]].difficulty.easy.Mind.DEFAULT_ENEMY_USEC = false;
				gv.botTypes[RogueTypes[rogue]].difficulty.normal.Mind.DEFAULT_ENEMY_USEC = false;
				gv.botTypes[RogueTypes[rogue]].difficulty.hard.Mind.DEFAULT_ENEMY_USEC = false;
				gv.botTypes[RogueTypes[rogue]].difficulty.impossible.Mind.DEFAULT_ENEMY_USEC = false;
			}
		}
	}
