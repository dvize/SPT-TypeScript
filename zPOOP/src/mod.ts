...
//export this constructor to declare public variables used in other classses.
...
export class globalValues
{
	...
	public static GlobalSessionID: string;
	public static profileOrigRole: string;
	public static profileOrigSide: string;
	public static scavAltRolesPickList: any;

	public static modName: string = "Fin-AITweaks";
...
class AITweaks implements IPreAkiLoadMod, IPostAkiLoadMod {
	...
	preAkiLoad(container: DependencyContainer): void {
		...
		staticRouterModService.registerStaticRouter(`StaticAkiGameStart${globalValues.modName}`, [{
			...
			action: (url, info, sessionId, output) => {
				globalValues.GlobalSessionID = sessionId;
				globalValues.profileOrigRole = AITweaks.clone(globalValues.profileHelper.getPmcProfile(sessionId).Info.Settings.Role);
				globalValues.profileOrigSide = AITweaks.clone(globalValues.profileHelper.getPmcProfile(sessionId).Info.Side);
		...
		if (!globalValues.config.disableAllAIChanges) {
			...
			container.afterResolution("BotGenerationCacheService", (_t, result: BotGenerationCacheService) => {
				result.storeBots = LegendaryPlayer.storeBots;
			}, { frequency: "Always" });
		}
	}
	...
	postAkiLoad(container: DependencyContainer): void {
		...
		globalValues.Logger.info(`Loading: ${globalValues.modName}`);
		...
		globalValues.Logger.info("Fin's AI Tweaks: Finished")
	}

	setupInitialValues(container) {
		globalValues.Logger.info('FINs AI TWEAKS: SetupInitialValues');
		...
		globalValues.baseAI = AITweaks.clone(globalValues.botTypes.pmcbot.difficulty.hard);
	...
	onRaidSave(url, info, sessionId, output) {
		...
		globalValues.Logger.info('FINs AI TWEAKS: onRaidSave')
	...
	runOnGameStart(url, info, sessionId): void {
		...
		globalValues.GlobalSessionID = sessionId //Make the player's ID accessible at any point
